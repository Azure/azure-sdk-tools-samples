    ...
    [string] $XmlDwConfigFile)
{
    ...
    [string] $XStoreStoragePartitionNodeString= "XStoreStorageInitializePartition"
    [string] $XStoreStorageMountNodeString = "XStoreStorageMountConfig"
    [string] $SqlServerNodeString = "SqlServerConfig"
    [string] $SqlStartUpParametersNodeString = "StartupParameters"
    [string] $SqlSpConfigureParametersNodeString = "SpConfigureOptions"
    [string] $SqlSpConfigureNameNodeString = "Name"
    [string] $SqlStartUpParametersFlagNodeString = "Flag"
    [string] $SqlSpConfigureOptionNodeString = "Option"
    [string] $SqlDatabaseConfigNodeString = "SqlDatabaseconfig"
    [string] $SqlDatabaseDatabaseNodeString = "Database"
    [hashtable] $XmlConfigObject = @{}
    ...
    if($VmConfig -eq $null)
    {
        ...
        else
        {
            ...
            {
                [string] $ConfigDetail = "Id = `"$IaasVmConfiguration.Id.ToString()`" NumberOfCores = `"$IaasVmConfiguration.NumberOfCores.ToString()`" ServerMemoryGB = `"$IaasVmConfiguration.ServerMemoryGB.ToString()`" NumberOfDataDisks = `"$IaasVmConfiguration.NumberOfDataDisks.ToString()`" NumberOfLogDisks = `"$IaasVmConfiguration.NumberOfLogDisks.ToString()`""
    ...
    #   UseMaximumSize                : True
    #   DataDiskFileSystemLabelPrefix : Data
    #   LogDiskFileSystemLabelPrefix  : Log
    #   AllocationUnitSizeKB          : 64
    #   FileSystem                    : NTFS
    foreach( $StorageAttribute in  $DwIaasConfigGeneral.$XStoreStorageRootNodeString.$XStoreStoragePartitionNodeString.Attributes)
    {
         $XmlConfigObject.Add($StorageAttribute.Name,$StorageAttribute.Value)
    }
    
    # Get the mount path info :
    #   MountPointRoot
    #   DataDiskMountPrefix
    #   LogDiskMountPrefix
    foreach( $StorageAttribute in  $DwIaasConfigGeneral.$XStoreStorageRootNodeString.$XStoreStorageMountNodeString.Attributes)
    {
         $XmlConfigObject.Add($StorageAttribute.Name,$StorageAttribute.Value)
    }
    
    # Get the Sql server startup parameters
    #  The Sql startup flags are stored in an array
    #  Each array will have a hash table , the hash table will store :
    #   Name
    #   Type
    #   Value
    # For more info check StartupParameters in $XmlDwConfigFile
    [array] $SqlStartUpFlagsArray = @()
    foreach( $SqlStartupFlag in  $DwIaasConfigGeneral.$SqlServerNodeString.$SqlStartUpParametersNodeString.$SqlStartUpParametersFlagNodeString)
    {
         [hashtable] $SqlStartupFlagObject = @{}
         foreach( $SqlstartupflagAttribute in  $SqlStartupFlag.Attributes)
         {
            $SqlStartupFlagObject.Add($SqlstartupflagAttribute.Name,$SqlstartupflagAttribute.Value)
         }
         $SqlStartUpFlagsArray += $SqlStartupFlagObject
    }
    $XmlConfigObject.Add($SqlStartUpParametersNodeString,$SqlStartUpFlagsArray)
    
    # Get the Sql server sp_configure parameters
    #  Parameters are stored in a similar manner as SQL startup flags
    # For more info check StartupParameters in $XmlDwConfigFile
    [array] $SqlSpConfigureArray = @()
    foreach( $SqlSpConfigure in  $DwIaasConfigGeneral.$SqlServerNodeString.$SqlSpConfigureParametersNodeString.$SqlSpConfigureOptionNodeString)
    {
         [hashtable] $SqlSpConfigureObject = @{}
         foreach( $SqlSpConfigureAttribute in  $SqlSpConfigure.Attributes)
         {
            $SqlSpConfigureObject.Add($SqlSpConfigureAttribute.Name,$SqlSpConfigureAttribute.Value)
         }
         $SqlSpConfigureArray += $SqlSpConfigureObject
    }
    $XmlConfigObject.Add($SqlSpConfigureParametersNodeString,$SqlSpConfigureArray)
    
    # Get the Sql server sp_configure parameters
    #  Parameters are stored in a similar manner as SQL startup flags
    # For more info check StartupParameters in $XmlDwConfigFile
    [array] $SqlDatabaseConfigArray = @()
    foreach( $SqlDatabaseConfig in  $DwIaasConfigGeneral.$SqlServerNodeString.$SqlDatabaseConfigNodeString.$SqlDatabaseDatabaseNodeString)
    {
         [hashtable] $SqlDatabaseConfigObject = @{}
         foreach( $SqlDatabaseConfigAttribute in  $SqlDatabaseConfig.Attributes)
         {
            $SqlDatabaseConfigObject.Add($SqlDatabaseConfigAttribute.Name,$SqlDatabaseConfigAttribute.Value)
         }
         $SqlDatabaseConfigArray += $SqlDatabaseConfigObject
    }
    
    $XmlConfigObject.Add($SqlDatabaseConfigNodeString,$SqlDatabaseConfigArray)
    
    
    [string] $DwConfigRootNodeString = "DwIaasConfig"
    [string] $DwConfigLogPathNodeString = "LogPaths"
    [hashtable] $XmlLogFilePathsObject = @{}
    [xml] $XmlConfig = gc $XmlDwConfigFile 
 
    foreach( $VmAttribute in  $XmlConfig.$DwConfigRootNodeString.$DwConfigLogPathNodeString.Attributes)
    {
         $XmlConfigObject.Add($VmAttribute.Name,$VmAttribute.Value)
    }
    ...
    [string] $WindowsAzurePsModulePath)
{
    if (Test-Path $WindowsAzurePsModulePath -ErrorAction SilentlyContinue)
    {
        try 
        {
            ...
            if($?)
            {
                Write-LogInfo -FunctionName $MyInvocation.MyCommand.Name -Message  " Importing Windows Azure PowerShell module from $WindowsAzurePsModulePath succeeded" -ForegroundColor "Green"
            }
        }
        catch [System.Management.Automation.ActionPreferenceStopException],[System.Net.WebException],[System.IO.IOException],[System.Management.Automation.CommandNotFoundException] 
        {
...
Function Validate-AzurePublishSettingsFile (
    ...
    [string] $PublishSettingsFile)
{
    Write-LogInfo -FunctionName $MyInvocation.MyCommand.Name -Message  " Start"
    ...
    [string] $ServiceName)
{
    ...
    Write-LogInfo -FunctionName $MyInvocation.MyCommand.Name -Message  " Start create Azure Affinity group"
        ...
        if((Get-AzureService -ServiceName $ServiceName  -ErrorAction silentlycontinue).AffinityGroup)
        {
            ...
            else
            {
                ...
                Please do one of the following:
                    ...
                    2) To use an existing Azure service don't provide the affinity group name or use the affinity group currently used by the service"
                exit -1
            }
        }
    ...
    [string] $AzureAffinityGroup)
{
    ...
    Write-LogInfo -FunctionName $MyInvocation.MyCommand.Name -Message  " Start create Azure storage account"
    ...
    [string] $SubscriptionName)
{
    ...
    #
    [Bool] $DoVerificationLoop = $false
    
    if (($SubscriptionName -eq $null) -or ($SubscriptionName.length -eq 0))
    {
        $DoVerificationLoop = $true
    }
    elseif((Get-AzureSubscription -SubscriptionName $SubscriptionName -ErrorAction SilentlyContinue) -eq $null)
    {
        $DoVerificationLoop = $true
    }
        
    if($DoVerificationLoop -eq $true)
    {
        do{
            Write-Host "Azure subscription $SubscriptionName not found!" -ForegroundColor red
            Write-Host "Please enter the subscrption name you wish to use:"
            [hashtable] $SubscriptionNameHT = @{}
            $count = 0
            $SubscriptionName = $null
            foreach ( $subscription in Get-AzureSubscription)
            {
                $internalSubscriptionName   = $subscription.SubscriptionName
                $SubscriptionNameHT.Add($internalSubscriptionName,1)
                $count                      += 1
                Write-Host "$count.     $internalSubscriptionName"
            }
            $SubscriptionName = Read-Host "Please enter SubscriptionName:"

            # Check that user entered the correct subscription name
            if (!($SubscriptionNameHT.Contains($SubscriptionName)))
            {
                Write-Host "Please enter SubscriptionName from provided list"
                continue
            }
            
            if ((Get-AzureSubscription -SubscriptionName $SubscriptionName) -ne $null)
            {
                break
            }
        }
        while (((Get-AzureSubscription -SubscriptionName $SubscriptionName) -eq $null) -and ( @(Get-AzureSubscription).count -gt 0) )
    }
...
{
    Write-LogInfo -FunctionName $MyInvocation.MyCommand.Name -Message  " Start create Azure VM"
    ...
    { 
        ...
        # Verify the azure subscription name
        #
        if((Get-AzureSubscription -SubscriptionName $SubscriptionName) -eq $null)
        {
            do{
                Write-Host "Azure subscription $SubscriptionName not found!" -ForegroundColor red
                Write-Host "Please enter the subscrption name you wish to use:"
        ...
        {
            throw "Azure subscription $SubscriptionName not found!"
        }
    ...
    { 
        ...
        { 
            ...
            if ($vm -ne $null)  
            { 
                Remove-AzureVM -ServiceName $ServiceName -Name $VMname -ErrorAction Stop 
            } 
            
            $CloudService = Get-AzureService -ServiceName $ServiceName 
...
[string] $XmlDwConfigFile = Join-Path  (split-path $MyInvocation.MyCommand.Path) "DwIaasConfigGeneral.xml"
