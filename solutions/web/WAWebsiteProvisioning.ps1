...
#This function takes the storage account name and location and verifies to make sure that the storage 
#account doesnt exist and then creates the storage accoun
Function CreateStorageAccount($StorageAccountName,$Location)
{
    
    $storageResult = Get-AzureStorageAccount | Where-Object {$_.StorageAccountName -eq $StorageAccountName }
    ...
    #$connectionString = "BlobEndpoint=http://{0}.blob.core.windows.net/;QueueEndpoint=http://{0}.queue.core.windows.net/;TableEndpoint=http://{0}.table.core.windows.net/;AccountName={0};AccountKey={1}" -f `, $key.Primary
    
    # when call Get-AzureStorageKey both primary and seconrady key is returned but we are only using the primary key.
    $appSettings = @{"StorageAccountName" = $StorageAccountName;"StorageAccountAccessKey" = $storageAccountKey.Primary}

    Return @{AppSettings=$appSettings}
}


#  Creating a Windows Azure SQL DB that will be linked to the Website.
Function CreateDB ($Location,$AppDatabaseName, $UserName, $Password, $RuleName, $FirewallRuleName, $StartIPAddress, $EndIPAddress )
{
    ...
    New-AzureSqlDatabaseServerFirewallRule -ServerName $databaseServer.ServerName -RuleName "AllowUserIP" -StartIpAddress $StartIPAddress -EndIpAddress $EndIPAddress
...
# Get the public IP address associated with the user's workstation for Azure rule 1 (user have access)
